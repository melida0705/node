...
{
  con.query("SELECT  * FROM orders INNER JOIN food ON food.food_id=order_details.order_food_id  where order_customer_username='"+req.params.customerid+"' and order_shipped='0' and order_preparation='0' and order_delivered='0'" ,function(error,rows,fields){
      if(error) console.log(error);
      else
      {   console.log("Radim");
          console.log(rows);
